...
export class NewDocShapeTool extends StateNode {
  ...
  override onPointerUp(_info: TLPointerEventInfo) {
    ...
    if (!ctx) {
      console.warn("[tldraw5] NewDocTool: context not set for this editor");
      this.editor.setCurrentTool("select");
      return;
    }
    ...
    if (!datatypeId) {
      console.warn("[tldraw5] NewDocTool: no datatype selected");
      this.editor.setCurrentTool("select");
      return;
    }
    ...
    void (async () => {
      try {
        ...
        console.error("[tldraw5] new doc creation failed:", err);
        if (editor.getShape(placeholderId)) {
          editor.updateShape({
            id: placeholderId,
            type: PATCHWORK_DOC_SHAPE_TYPE,
